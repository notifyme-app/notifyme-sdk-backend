...
public class CryptoWrapper {
  ...
  public KeyPair keyGen() {
    final var msk = new Fr();
    msk.setByCSPRNG();
    final var mpk = new G2();
    Mcl.mul(mpk, cryptoUtilV3.baseG2(), msk);
    return new KeyPair(Key.fromBytes(mpk.serialize()), Key.fromBytes(msk.serialize()));
  }
  ...
  private G1 getSecretKeyForIdentity(byte[] ctxha, byte[] identity, byte[] pskidL) {
  ...
  public class CryptoUtilV3 {
    ...
    public byte[] generateIdentity(QRCodePayload qrCodePayload, long startOfInterval) {
  ...
  public class CryptoUtilV2 {
    ...
    public void calculateSecretKeyForIdentityAndIdentity(
        PreTraceWithProof preTraceWithProof, int affectedHour, TraceKey traceKey)
        throws InvalidProtocolBufferException {
      ...
      G1 secretKeyForIdentity =
          getSecretKeyForIdentity(
              ...
              preTrace.getIdentity().toByteArray(),
              preTrace.getPartialSecretKeyForIdentityOfLocation().toByteArray());
      var qrCodeContent = QrCodeContent.parseFrom(preTraceWithProof.getInfo());
    ...
    private byte[] generateIdentity(
        QrCodeContent qrCodeContent, byte[] nonce1, byte[] nonce2, int hour) {
